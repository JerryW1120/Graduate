    ...
    if choice_of_laplace == 'y':
        ...
        for movie in top_sim_user_1_data.keys():
            if movie not in top_sim_user_2_data.keys():
                ...
                predict_score = (float(top_sim_user_1_data[movie]) * weight1 + float(top_sim_user_2_data[movie]) * weight2) / (weight1 + weight2)
        ...
        for movie in top_sim_user_1_data.keys():
            print("电影：", movie, "    分数：", top_sim_user_1_data[movie])
            if movie not in top_sim_user_2_data.keys():
