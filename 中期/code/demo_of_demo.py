...
"""计算任何两位用户之间的相似度，由于每位用户评论的电影不完全一样
"""
...
def pearson_sim_pro(user1, user2):
    ...
    # 计算平均数
    ave1 = sum([float(user1_data[movie]) for movie in user1_data]) / n
    ave2 = sum([float(user2_data[movie]) for movie in user2_data]) / n

    # 计算分子
    num = sum((float(user1_data[movie]) - ave1) *
              (float(user2_data[movie]) - ave2) for movie in common)
    # sum1Sq = sum([pow(float(user1_data[movie]),2) for movie in common])
    # sum2Sq = sum([pow(float(user2_data[movie]),2) for movie in common])

    # 计算分母参数以及分母
    den1 = sum(pow(float(user1_data[movie]) - ave1, 2) for movie in common)
    den2 = sum(pow(float(user2_data[movie]) - ave2, 2) for movie in common)
    den = sqrt(den1 * den2)
...
def recommend_with_predict(user):
    ...
    print(RES)
    # 得到用户名和相似值，为下一步计算做准备
    message1 = top10_simliar(user, choice)[0]
    top_sim_user_1 = message1[0]
    # 计算预测值的权重
    weight1 = message1[1]
    print("最相似用户1的名字和相似度：\n", top_sim_user_1, "\n", weight1, "\n")
    message2 = top10_simliar(user, choice)[1]
    ...
    if choice_of_laplace == 'y':
        sensitivety = int(input("输入敏感度："))
        epsilon = int(input("输入差分隐私参数："))

        for movie in top_sim_user_1_data.keys():
            if movie not in top_sim_user_2_data.keys():
                predict_score = laplace_mech(float(top_sim_user_1[1]), sensitivety, epsilon)
                ...
                predict_score = laplace_mech(predict_score, sensitivety, epsilon)
                predict_movies.append((movie, predict_score))
        for movie in top_sim_user_2_data:
            if movie in top_sim_user_1_data:
                continue
            else:
                predict_score = laplace_mech(float(top_sim_user_2[1]), sensitivety, epsilon)
                predict_movies.append((movie, predict_score))
    else:
        for movie in top_sim_user_1_data.keys():
            if movie not in top_sim_user_2_data.keys():
