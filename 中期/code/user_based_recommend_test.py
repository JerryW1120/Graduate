...
data = data.to_csv('/Users/jerryw/Desktop/ml-latest-small/data.csv', index=False)

file = open("/Users/jerryw/Desktop/ml-latest-small/data.csv", 'r', encoding='UTF-8')
...
def recommend_with_predict(user):
    ...
    # 计算评分平均数
    ave1 = sum([float(top_sim_user_1_data[movie]) for movie in top_sim_user_1_data]) / len(top_sim_user_1_data)
    ave2 = sum([float(top_sim_user_2_data[movie]) for movie in top_sim_user_2_data]) / len(top_sim_user_2_data)
    ave = sum([float(user_data[movie]) for movie in user_data]) / len(user_data)
    predict_movies = []
    ...
    if choice_of_laplace == 'y':
        ...
        for movie in top_sim_user_1_data.keys():
            if movie not in top_sim_user_2_data.keys():
                ...
                predict_score = float(top_sim_user_1_data[movie]) + np.random.laplace(0, beta, 1)
                predict_movies.append((movie, float(predict_score)))
            else:
                # 如果用户2看过该电影，预测评分为两名用户的评分分别乘以各自的权重（相似度）
                predict_score = ave + ((float(top_sim_user_1_data[movie]) - ave1) * weight1 + (float(top_sim_user_2_data[movie]) - ave2) * weight2) / (weight1 + weight2)
        ...
        for movie in top_sim_user_2_data:
            ...
            if movie in top_sim_user_1_data:
                ...
                predict_score = float(top_sim_user_2_data[movie]) + np.random.laplace(0, beta, 1)
                predict_movies.append((movie, float(predict_score)))
    else:
        for movie in top_sim_user_1_data.keys():
            # print("电影：", movie, "    去中心化之后的分数：", float(top_sim_user_1_data[movie]) - ave1)
            if movie not in top_sim_user_2_data.keys():
                predict_movies.append((movie, float(top_sim_user_1_data[movie])))
            else:
                predict_score = ave + ((float(top_sim_user_1_data[movie]) - ave1) * weight1 + (float(top_sim_user_2_data[movie]) - ave2) * weight2) / (weight1 + weight2)
                predict_movies.append((movie, predict_score))
            # print(predict_movies)
            # print('\n')
        for movie in top_sim_user_2_data.keys():
            if movie in top_sim_user_1_data.keys():
                continue
            else:
                predict_movies.append((movie, float(top_sim_user_2_data[movie])))
            # print(predict_movies)
            # print('\n')
