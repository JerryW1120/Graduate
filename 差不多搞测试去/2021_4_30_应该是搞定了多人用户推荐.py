...
def pearson_sim_pro(user1, user2):
    ...
    # 计算分子,9742是电影数目
    sum_of_product_of_xy = 9742 * sum(float(user1_data[movie]) * float(user2_data[movie]) for movie in common)
    ...
    sum1Sq = 9742 * sum([pow(float(user1_data[movie]), 2) for movie in user1_data])
    sum2Sq = 9742 * sum([pow(float(user2_data[movie]), 2) for movie in user2_data])
    den1 = sum1Sq - sum1
    den2 = sum2Sq - sum2
    den = sqrt(den1 * den2)
...
def recommend_with_predict(user):
    ...
    # 读取电影名字
    file = open("/Users/jerryw/Desktop/ml-latest-small/movies.csv", 'r', encoding='UTF-8')
    movies = []
    for line in file.readlines()[1:9743]:
        line = line.strip().split(',')
