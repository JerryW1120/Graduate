...
def recommend_with_predict(user):
    ...
    # 存放RSME和MAE值，为输出表格做准备，其中RMSE_list, MAE_list是二维列表，列(column)是隐私预算，行(index)是敏感度
    RMSE_list = []
    MAE_list = []
    sensitivety_list = []
    epsilon_list = []

    while sensitivety < 5.01:
       # 具体的值存储在子列表中
        value_of_RMSE = []
        value_of_MAE = []
        sensitivety_list.append(sensitivety)
        RMSE_list.append(value_of_RMSE)
        MAE_list.append(value_of_MAE)
        epsilon = 0.1
        while epsilon < 4.05:
    # choice_of_laplace = input("要不要加差分隐私？\n“y”代表确认")
            choice_of_laplace = baseline
            ...
            print(user, "在敏感度为", sensitivety, "，隐私预算为", epsilon, "的均方根误差为：", value_of_rmse, "平均绝对误差为：", value_of_mae)
            
        
            # 按照评分从高到低排序
            # predict_movies.sort(key = lambda val: val[1])
            # predict_movies.reverse()
            # print("在这个情况下推荐的电影为：", predict_movies[:10])
            if baseline == 0:
                baseline = 1
                print("以上为不加差分隐私的情况\n")
            else:
                value_of_RMSE.append(value_of_rmse)
                value_of_MAE.append(value_of_mae)
                epsilon += 0.1
    ...
    table_of_RMSE = pd.DataFrame(RMSE_list, index = sensitivety_list, columns = epsilon_list)           
    # print("用户", user, "在不同敏感度和隐私预算的情况下得到的RMSE如下\n", table_of_RMSE)
    table_of_MAE = pd.DataFrame(MAE_list, index = sensitivety_list, columns = epsilon_list)
    # print("\n用户", user, "在不同敏感度和隐私预算的情况下得到的MAE如下\n", table_of_MAE)
    return table_of_RMSE, table_of_MAE
...
if __name__ == '__main__':
    ...
    for sheet in sheets:          
        ...
        Recommendations[0].to_excel(writer_of_RMSE, sheet)
        Recommendations[1].to_excel(writer_of_MAE, sheet)
        # print("用户", count + 1 ,"的推荐电影:\n", Recommendations)
        print("\n")
        user += 1   
    writer_of_RMSE.save()
    writer_of_MAE.save()
